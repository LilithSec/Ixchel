package Ixchel::Actions::sagan_rules_in_place;
...
Ixchel::Actions::sagan_rules_in_place - Regen the Sagan rules file based on rules.yaml file from the rules dir.
...
ixchel -a sagan_rules_in_place [B<-w>][B<-i> <instance>]

ixchel -a sagan_rules_in_place --diff [B<-i> <instance>]
...
Create between what is in place and what it will be updated to.

Not compatibile with --diff.
...
sub action_extra {
	...
	if ( $self->{opts}{w} && $self->{opts}{diff} ) {
		$self->status_add( error => 1, status => '-w and --diff may not be used together' );
		return;
	}
